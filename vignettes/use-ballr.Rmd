...
if (packageVersion("janitor") <= "0.3.1") {
  player_stats <- dplyr::mutate(player_stats, e_fg_percent = efgpercent)
}

p <- ggplot2::ggplot(data = player_stats,
            aes(x = age, y = e_fg_percent, group = player))
p + ggplot2::geom_line(alpha = .25) +
  ...
  ggplot2::geom_line(data = dplyr::filter(player_stats, player == "Anthony Davis"),
            aes(x = age, y = e_fg_percent), size = 1, col = "#1f78b4") +
  ...
  ggplot2::geom_line(data = dplyr::filter(player_stats, player == "DeMarcus Cousins"),
            aes(x = age, y = e_fg_percent), size = 1, col = "#33a02c") +
